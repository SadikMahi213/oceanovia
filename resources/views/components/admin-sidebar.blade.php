...
$nav = [
    ...
    ['label' => 'Procurements', 'route' => 'admin.procurements.index', 'icon' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>'],
    ...
    ['label' => 'Notifications', 'route' => 'admin.notifications.index', 'badge' => $unreadNotifications, 'icon' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>'],
    ...
    // ─── Communication ───────────────────────────────────────
    ['label' => 'Email Sender', 'route' => 'admin.emails.index', 'icon' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207"/></svg>'],
                ...
                <a href="{{ route($item['route']) }}"
                   ...
                   :title="!expanded ? '{{ $item['label'] }}' : ''">
                    ...
                    @if(!empty($item['badge']) && $item['badge'] > 0)
                        <span x-show="expanded" x-cloak class="ml-auto shrink-0 inline-flex items-center justify-center px-1.5 py-0.5 rounded-full bg-red-500 text-white text-[10px] font-bold min-w-[1.25rem]">{{ $item['badge'] }}</span>
                        <span x-show="!expanded" x-cloak class="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" :title="'{{ $item['badge'] }} unread'"></span>
                    @endif
                </a>
            @endforeach
        </nav>
