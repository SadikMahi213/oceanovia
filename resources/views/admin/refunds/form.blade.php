                    ...
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            <div class="flex items-center gap-3">
                                <a href="{{ route('admin.refunds.show', $refund) }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                                </a>
                                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Refund Action</h1>
                                @php
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                                ...
                                <div class="p-5 space-y-4">
                                    <div class="flex items-center justify-between">
                                        <span class="text-sm text-gray-500 dark:text-gray-400">Refund ID</span>
                                        <span class="text-sm font-mono text-gray-900 dark:text-white">#{{ $refund->id }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $refund->user?->name ?? '—' }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-sm text-gray-500 dark:text-gray-400">Customer Email</span>
                                        <span class="text-sm text-gray-700 dark:text-gray-300">{{ $refund->user?->email ?? '—' }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-start justify-between">
                                        ...
                                        <span class="text-sm text-gray-700 dark:text-gray-300 max-w-[300px] text-right">{{ $refund->reason }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                ...
                                <div class="p-5 space-y-4">
                                    @if($refund->status === 'pending')
                                        <form method="POST" action="{{ route('admin.refunds.approve', $refund) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-xl transition-colors">
                                    ...
                                    <form method="POST" action="{{ route('admin.refunds.reject', $refund) }}" class="space-y-3">
                                        ...
                                        @method('PATCH')
                                        <div>
                                            <x-input-label for="rejection_reason" value="Admin Notes / Rejection Reason" />
                                            <textarea id="rejection_reason" name="reason" rows="4"
                                                ...
                                                placeholder="Enter notes or rejection reason...">{{ old('reason') }}</textarea>
                                            <x-input-error :messages="$errors->get('reason')" class="mt-1" />
                                        </div>
                                        <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-xl transition-colors">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                            {{ $refund->status === 'pending' ? 'Reject Refund' : 'Save Notes' }}
                                        </button>
                                    </form>
                                </div>
                            </div>
