                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">In-app alerts (new registrations and account activity)</p>
                        </div>
                    ...
                    <div class="space-y-3">
                        @forelse($notifications as $notification)
                            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden {{ !$notification->read_at ? 'border-l-4 border-l-market-500' : '' }}">
                                <div class="p-5">
                                    <div class="flex items-start gap-4">
                                        <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m0 0A5.99 5.99 0 0115 21"/></svg>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-start justify-between gap-2">
                                                <div class="flex items-center gap-2 min-w-0">
                                                    ...
                                                    @endif
                                                    <h3 class="text-sm font-semibold text-gray-900 dark:text-white {{ !$notification->read_at ? 'font-bold' : '' }}">{{ $notification->title }}</h3>
                                                </div>
                                                <span class="text-xs text-gray-500 dark:text-gray-400 shrink-0">{{ $notification->created_at->diffForHumans() }}</span>
                                            </div>
                                            <div class="flex items-center gap-2 flex-wrap mt-1">
                                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ data_get($notification->data, 'email') }}</p>
                                                <span class="text-xs text-gray-400 dark:text-gray-500">·</span>
                                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 capitalize">{{ data_get($notification->data, 'role_type') ?? '—' }}</span>
                                            </div>
                                            <div class="flex items-center gap-4 mt-2">
                                                @if($notification->link)
                                                    <a href="{{ $notification->link }}" class="text-xs font-medium text-market-600 dark:text-market-400 hover:underline">View User</a>
                                                @endif
                                                @if(!$notification->read_at)
