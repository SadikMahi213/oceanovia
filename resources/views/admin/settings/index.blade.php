                ...
                <div class="flex-1 min-w-0">
                    ...
                    @php
                        ...
                        $groupIcons = [
                            ...
                            'shipping' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"/></svg>',
                            'commission' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
                            'appearance' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01"/></svg>',
                            'seo' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>',
                        ];
                    @endphp
                    ...
                    @forelse($settings as $group => $groupSettings)
                        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden mb-6">
                            <div class="p-5 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3">
                                <span class="text-gray-500 dark:text-gray-400">{{ $groupIcons[$group] ?? '' }}</span>
                                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $groupLabels[$group] ?? ucfirst($group) }}</h2>
                            </div>
                            <form method="POST" action="{{ route('admin.settings.update') }}">
                                ...
                                <div class="p-5 space-y-4">
                                    @foreach($groupSettings as $setting)
                                        <div>
                                            <x-input-label :for="'settings_' . $setting->key" :value="ucfirst(str_replace('_', ' ', $setting->key))" />
                                            <input type="text" id="settings_{{ $setting->key }}" name="settings[{{ $setting->key }}]" value="{{ old('settings.' . $setting->key, $setting->value) }}"
                                ...
                                <div class="p-5 border-t border-gray-100 dark:border-gray-700 flex justify-end">
                                    <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-market-600 hover:bg-market-700 text-white text-sm font-medium rounded-xl transition-colors">
                                        ...
                                        Save {{ $groupLabels[$group] ?? ucfirst($group) }}
                                    </button>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
                            <div class="p-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 dark:text-white">No settings found</p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Settings will appear here once they are created</p>
                                    </div>
                                </div>
                            </div>
                        </div>
